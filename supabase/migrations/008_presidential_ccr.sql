-- Migration 008: Presidential signing + CCR constitutionality tracking

-- ── 1. New columns on laws ────────────────────────────────────────────
ALTER TABLE laws
  ADD COLUMN IF NOT EXISTS presidential_status TEXT
    CHECK (presidential_status IN ('promulgat', 'retrimis', 'sesizat_ccr')),
  ADD COLUMN IF NOT EXISTS presidential_date DATE,
  ADD COLUMN IF NOT EXISTS ccr_decision TEXT
    CHECK (ccr_decision IN ('constitutional', 'neconstitutional', 'partial_neconstitutional')),
  ADD COLUMN IF NOT EXISTS ccr_date DATE;

-- ── 2. Rebuild law_status view to expose new fields ───────────────────
